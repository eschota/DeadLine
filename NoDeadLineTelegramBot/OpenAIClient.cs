                                                    ...
                                                    { "function", new { type = "string", @enum = new[] { "GenerateImages" } } },
                                                    {
                                                        "parameters", new
                                                        {
                                                            ...
                                                            properties = new Dictionary<string, object>
                                                            {
                                                                ...
                                                                { "imagePrompts", new { type = "array", items = new { type = "string" }, description = "Массив промптов для генерации изображений. обязательно на английском языке. Описание деталей и смысла изображения длиной максимум 30 слов, промпт должен описывать Сцену происходящего на будущем изображении. промпты для каждого изображения должны соответствовать общему ответу JustAnswer, все промпты на английском языке, описывают сцену и действие и главных действующих персонажей." } }
                                                            },
                                                ...
                                                properties = new Dictionary<string, object>
                                                {
                                                    { "function", new { type = "string", @enum = new[] { "JustAnswer" } } },
                                                    {
                                                        "parameters", new
                                                        {
                                                            ...
                                                            properties = new Dictionary<string, object>
                                                            {
                                                                { "prompt", new { type = "string", description = "Ответ на запрос пользователя. Ничего не объясняй, строго пиши только ответ, без обсуждений и рекомендаций, если просят создать историю и тематику, то просто пишешь историю и тематику. и если просят сгенерировать изображения просто отвечай что сделаешь всё и опишешь как понял задачу." } }
                                                            },
                                                            required = new[] { "prompt" },
                                                            additionalProperties = false
                                                        }

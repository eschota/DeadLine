...
Shader "AtmosphereGlowShader"
{
	Properties
	{
		...
		[ASEBegin]_GlowScaleOutside("GlowScale Outside", Range( 0 , 1)) = 0.6
		...
		[ASEEnd]_AtmosphereColor("AtmosphereColor", Color) = (0.319749,0.5665256,0.7137255,1)
	...
	SubShader
	{
		...
		Pass
		{
			...
			float _GlowScaleOutside;
			float _GlowScaleInside;
			float _DayNightContrast;
			float _DarkSideMult;
						 ...
						 ) : SV_Target
			{
				...
				float fresnelNdotV101 = dot( WorldNormal, WorldViewDirection );
				...
				float dotResult17 = dot( _MainLightPosition.xyz , WorldNormal );
				...
				float3 BaseColor = _AtmosphereColor.rgb;
				float3 Normal = float3(0, 0, 1);
				float3 Emission = _AtmosphereColor.rgb;
				float3 Specular = 0.5;
				float Metallic = 0;
				float Smoothness = 0.5;
				float Occlusion = 1;
				float Alpha = lerpResult111.r;
		...
		Pass
		{
						 ...
						 ) : SV_TARGET
			{
				...
				float clampResult104 = clamp( fresnelNode101 , 0.0 , 1.0 );
				float temp_output_102_0 = ( ( 1.0 - clampResult103 ) * clampResult104 );
				float dotResult17 = dot( _MainLightPosition.xyz , ase_worldNormal );
				float4 temp_cast_0 = (dotResult17).xxxx;
				float4 clampResult96 = clamp( CalculateContrast(_DayNightContrast,temp_cast_0) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float4 temp_cast_1 = (temp_output_102_0).xxxx;
				float4 lerpResult111 = lerp( ( temp_output_102_0 * clampResult96 ) , temp_cast_1 , _DarkSideMult);
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float temp_output_102_0 = ( ( 1.0 - clampResult103 ) * clampResult104 );
				float dotResult17 = dot( _MainLightPosition.xyz , ase_worldNormal );
				float4 temp_cast_1 = (dotResult17).xxxx;
				float4 clampResult96 = clamp( CalculateContrast(_DayNightContrast,temp_cast_1) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float4 temp_cast_2 = (temp_output_102_0).xxxx;
				float4 lerpResult111 = lerp( ( temp_output_102_0 * clampResult96 ) , temp_cast_2 , _DarkSideMult);
		...
		Pass
		{
						 ...
						 ) : SV_TARGET
			{
				...
				float3 ase_worldViewDir = ( _WorldSpaceCameraPos.xyz - WorldPosition );
				ase_worldViewDir = normalize(ase_worldViewDir);
				float fresnelNdotV57 = dot( IN.worldNormal, ase_worldViewDir );
				float fresnelNode57 = ( 0.0 + ( ( 1.0 - _GlowScaleOutside ) * 20.0 ) * pow( 1.0 - fresnelNdotV57, 10.0 ) );
				float clampResult103 = clamp( fresnelNode57 , 0.0 , 1.0 );
				float fresnelNdotV101 = dot( IN.worldNormal, ase_worldViewDir );
				float fresnelNode101 = ( 0.0 + ( _GlowScaleInside * 20.0 ) * pow( 1.0 - fresnelNdotV101, 3.0 ) );
				float clampResult104 = clamp( fresnelNode101 , 0.0 , 1.0 );
				float temp_output_102_0 = ( ( 1.0 - clampResult103 ) * clampResult104 );
				float dotResult17 = dot( _MainLightPosition.xyz , IN.worldNormal );
				float4 temp_cast_0 = (dotResult17).xxxx;
				float4 clampResult96 = clamp( CalculateContrast(_DayNightContrast,temp_cast_0) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float4 temp_cast_1 = (temp_output_102_0).xxxx;
				float4 lerpResult111 = lerp( ( temp_output_102_0 * clampResult96 ) , temp_cast_1 , _DarkSideMult);
...
Node;AmplifyShaderEditor.RangedFloatNode;112;4295.957,1228.718;Float;False;Property;_DarkSideMult;DarkSideMult;3;0;Create;True;0;0;0;False;0;False;0.022;0.22;0;1;0;1;FLOAT;0
...
WireConnection;156;2;144;0
WireConnection;156;6;111;0
ASEEND*/
//CHKSM=6ADC10BF38063F52C42328442A4680259DD4F520
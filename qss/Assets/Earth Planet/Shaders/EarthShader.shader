...
Shader "EarthShader"
{
	Properties
	{
		...
		[ASEBegin]_EarthMap("EarthMap", 2D) = "white" {}
		...
		[ASEEnd]_AtmosphereDarkSide("AtmosphereDarkSide", Range( 0 , 1)) = 0.423
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 _NightMap_ST;
			float4 _EarthMap_ST;
			float4 _EarthNormalMap_ST;
			float4 _WaterMask_ST;
			float4 _AtmosphereColor;
			float4 _NightLightsMap_ST;
			float _NightMapMult;
			...
			float _AtmosphereDarkSide;
			float _NightLightsMult;
			...
			sampler2D _NightMap;
			sampler2D _EarthMap;
			sampler2D _EarthNormalMap;
			sampler2D _WaterMask;
			sampler2D _NightLightsMap;
						 ...
						 ) : SV_Target
			{
				...
				float2 uv_NightMap = IN.ase_texcoord7.xy * _NightMap_ST.xy + _NightMap_ST.zw;
				float4 temp_output_18_0 = ( tex2D( _NightMap, uv_NightMap ) * _NightMapMult );
				float2 uv_EarthMap = IN.ase_texcoord7.xy * _EarthMap_ST.xy + _EarthMap_ST.zw;
				float dotResult17 = dot( _MainLightPosition.xyz , WorldNormal );
				float4 temp_cast_0 = (dotResult17).xxxx;
				float4 clampResult43 = clamp( CalculateContrast(_DayNightContrast,temp_cast_0) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				...
				float2 uv_EarthNormalMap = IN.ase_texcoord7.xy * _EarthNormalMap_ST.xy + _EarthNormalMap_ST.zw;
				float3 tex2DNode23 = UnpackNormalScale( tex2D( _EarthNormalMap, uv_EarthNormalMap ), 1.0f );
				...
				float2 uv_WaterMask = IN.ase_texcoord7.xy * _WaterMask_ST.xy + _WaterMask_ST.zw;
				...
				float fresnelNdotV96 = dot( WorldNormal, WorldViewDirection );
				...
				float2 uv_NightLightsMap = IN.ase_texcoord7.xy * _NightLightsMap_ST.xy + _NightLightsMap_ST.zw;
				
				float lerpResult54 = lerp( _LandSmoothness , _WaterSmoothness , tex2DNode24.r);
				

				float3 BaseColor = lerpResult4.rgb;
				float3 Normal = lerpResult51.rgb;
				float3 Emission = ( ( temp_output_21_0 * temp_output_18_0 ) + clampResult92 + ( tex2D( _NightLightsMap, uv_NightLightsMap ) * _NightLightsMult * temp_output_21_0 ) ).rgb;
				float3 Specular = 0.5;
				float Metallic = 0.0;
				float Smoothness = lerpResult54;
				float Occlusion = 1;
				float Alpha = 1;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float2 uv_NightMap = IN.ase_texcoord2.xy * _NightMap_ST.xy + _NightMap_ST.zw;
				float4 temp_output_18_0 = ( tex2D( _NightMap, uv_NightMap ) * _NightMapMult );
				float2 uv_EarthMap = IN.ase_texcoord2.xy * _EarthMap_ST.xy + _EarthMap_ST.zw;
				...
				float4 lerpResult65 = lerp( ( temp_output_63_0 * fresnelNode96 ) , ( dotResult17 * temp_output_63_0 ) , ( 1.0 - _AtmosphereDarkSide ));
				float4 clampResult92 = clamp( lerpResult65 , float4( 0,0,0,1 ) , float4( 1,1,1,1 ) );
				float2 uv_NightLightsMap = IN.ase_texcoord2.xy * _NightLightsMap_ST.xy + _NightLightsMap_ST.zw;
...
WireConnection;229;0;4;0
WireConnection;229;1;51;0
WireConnection;229;2;61;0
WireConnection;229;3;79;0
WireConnection;229;4;54;0
ASEEND*/
//CHKSM=376F75754702079BA0F3BFF6AECEC89512347D96